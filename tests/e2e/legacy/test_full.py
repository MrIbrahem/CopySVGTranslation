...
from tests.service_api import inject_translations
from tests.service_api import extract_translations
...
def test_inject_uses_existing_mapping(tmp_path: Path, target_svg: Path, fixtures_dir) -> None:
    ...
    translations = extract_translations(fixtures_dir / "source.svg")

    _output_dir = tmp_path / "outputs"
    _output_dir.mkdir(parents=True, exist_ok=True)
    output_file = _output_dir / target_svg.name

    tree, stats = inject_translations(
...
def test_extract_with_pathlib_path(fixtures_dir) -> None:
    ...
    result = extract_translations(source_path)
...
def test_extract_empty_svg(tmp_path: Path) -> None:
    ...
    result = extract_translations(empty_svg)
...
<svg xmlns="http://www.w3.org/2000/svg">
  <switch>
    ...
    result = extract_translations(multi_lang_svg)
...
def test_extract_with_case_insensitive_true(fixtures_dir) -> None:
    ...
    """
    result = extract_translations(fixtures_dir / "source.svg", case_insensitive=True)
...
def test_inject_multiple_operations(tmp_path: Path, target_svg: Path, fixtures_dir) -> None:
    ...
    output1.mkdir()
    tree1, stats1 = inject_translations(
        inject_file=target_svg,
        mapping=translations,
    ...
    output2.mkdir()
    tree2, stats2 = inject_translations(
        inject_file=target_svg,
        mapping=translations,
