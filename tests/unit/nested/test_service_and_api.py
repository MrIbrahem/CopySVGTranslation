...
def test_nested_structure_service_repair_in_place():
    root = etree.fromstring(_svg("""<text id="t1"><tspan><tspan style="font-weight: 700;">Bold</tspan></tspan></text>"""))
    service = NestedStructureService()
    tree = service.repair(root)
    assert isinstance(tree, etree._ElementTree)
    findings = service.analyze(root)
