...
Nested `<tspan>` and `<a>` nodes may require special handling. These operations are deliberately separate from `extract()` and `inject()`, so you can inspect or repair a document before proceeding. Full reference — including `analyze_nested()`, `repair_nested()` usage, strategies, and `RepairResult` fields.

Read: [docs/nested-structures.md](docs/nested-structures.md).
