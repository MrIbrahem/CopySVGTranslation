...
class SwitchProcessor:
    ...
    def process(
        ...
        # Check if any of our default texts has any translation in any language
        any_translation_found = False
        for src in default_texts:
            resolved = working_mapping.lookup(src, case_insensitive=self.config.case_insensitive)
            if resolved:
                any_translation_found = True
                break

        if not any_translation_found:
            return
